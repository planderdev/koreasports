...
export function setupShell(){if(['admin','design-system'].includes(document.body.dataset.page)){setupWorkspace();return;}const nav=r.content('navigation');setupHeaderNavigation(nav);
 ...
 document.querySelectorAll('[data-menu]').forEach(b=>b.onclick=()=>{
  ...
  modal('전체메뉴',`<nav class="menu-account-links" aria-label="회원 및 검색"><a href="${member?'/mypage.php':'/login.php'}">${icon(member?'user-line':'login-box-line')}${member?'마이페이지':'로그인'}</a><a href="/join.php">${icon('user-add-line')}회원가입</a><a href="/search.php">${icon('search-line')}통합검색</a></nav><nav class="all-menu" aria-label="전체 사이트 메뉴">${nav.map((n,i)=>`<section aria-labelledby="all-menu-title-${i}"><h3 id="all-menu-title-${i}">${esc(n.title)}</h3><div>${n.items.map(x=>x.children?.length?`<div class="all-menu-branch"><a class="all-menu-parent" href="${esc(x.url)}">${esc(x.title)}</a><ul>${x.children.map(child=>`<li><a href="${esc(child.url)}">${esc(child.title)}</a></li>`).join('')}</ul></div>`:`<a href="${esc(x.url)}">${esc(x.title)}</a>`).join('')}</div></section>`).join('')}</nav>`);
