...
export function businessPage(id, page) {
  ...
  if (id === 'business') {
    ...
    content = `<section class="business-fields" aria-labelledby="business-fields-title"><h2 id="business-fields-title">주요 사업 분야</h2>${data.fields.map((item, index) => `<article class="business-field"${item.id ? ` id="field-${esc(item.id)}"` : ''}><header>${number(index)}<h3>${esc(item.title)}</h3></header><div><h4>${esc(item.subtitle)}</h4><p>${esc(item.body)}</p>${item.facts ? `<dl class="business-facts">${item.facts.map(([label, value]) => `<div><dt>${esc(label)}</dt><dd>${esc(value)}</dd></div>`).join('')}</dl>` : ''}<ul class="business-points">${item.items.map(text => `<li>${esc(text)}</li>`).join('')}</ul>${item.notes ? `<ul class="business-notes">${item.notes.map(text => `<li>${esc(text)}</li>`).join('')}</ul>` : ''}${item.links ? `<div class="business-links">${links(item.links)}</div>` : ''}</div></article>`).join('')}</section><section class="business-cooperation"><h2>${esc(cooperation.title)}</h2><p>${esc(cooperation.body)}</p><div class="business-columns">${cooperation.items.map(item => `<article>${columnImage(item)}<h3>${esc(item.title)}</h3><p>${esc(item.body)}</p></article>`).join('')}</div></section>`;
  } else {
