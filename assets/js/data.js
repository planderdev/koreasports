// All records are synthetic. Replace approved copy here, not in page templates.
export const siteConfig = {name:'대한직장인체육회',englishName:'KOREAN WORKERS SPORTS ASSOCIATION',president:null,address:null,phone:null,email:null,logo:null,version:1,isDemo:true};
const page = (title,id) => ({title,url:`/page.php?id=${id}`,id});
const link = (title,url) => ({title,url});
export const navigation = [
 {title:'체육회소개',english:'ABOUT US',items:[page('회장인사말','greeting'),{...page('Vision','vision'),children:[page('Vision 1','vision-1'),page('Vision 2','vision-2')]},page('설립목적','purpose'),page('조직기구표','organization'),page('임원 & 위원회','committee'),page('연혁','history'),page('체육회 CI','ci'),page('정관','articles'),page('오시는길','directions')]},
 ...
 {title:'커뮤니티',english:'COMMUNITY',items:[link('Club Matching','/clubs.php'),link('우리 동호회 자랑','/board.php?category=동호회 소식'),link('이달의 우수동호회','/clubs.php?featured=1'),page('자주묻는질문','faq'),page('Q&A','qna'),link('자료실','/board.php?category=자료실'),page('안전개선제안','safety-proposal'),page('회원전용','members-only')]},
 {title:'후원참여',english:'TOGETHER',items:[link('후원금집행내역','/support.php?tab=reports'),link('기업파트너십','/support.php?tab=partnership'),link('모집안내/권리','/support.php?tab=rights'),link('후원하기','/support.php?tab=donate'),link('후원사현황','/support.php?tab=sponsors'),link('자원봉사 신청','/volunteers.php'),link('회원가입','/join.php')]}
];
...
export const heroSlides = [
 {id:'hero-1',eyebrow:'SPORTS CONNECTS US',title:'일하는 우리,\n스포츠로 하나 되다.',summary:'일상의 균형을 찾는 시간, 함께하는 스포츠에서 시작됩니다.',image:mediaAssets[0].image,alt:mediaAssets[0].alt,cta:'대회 참가신청',url:'/events.php'},
 {id:'hero-2',eyebrow:'YOUR NEXT CHALLENGE',title:'새로운 도전,\n함께라서 더 즐겁게.',summary:'골프부터 펜싱, 승마, 풋살까지. 나만의 무대를 만나보세요.',image:mediaAssets[1].image,alt:mediaAssets[1].alt,cta:'대회 살펴보기',url:'/events.php'},
 {id:'hero-3',eyebrow:'BETTER TOGETHER',title:'함께 뛰는 오늘,\n더 건강한 내일.',summary:'퇴근 후의 설렘, 취향이 같은 동료와 함께 만들어갑니다.',image:mediaAssets[3].image,alt:mediaAssets[3].alt,cta:'동호회 찾기',url:'/clubs.php'}
];
export const quickLinks = [ ['trophy-line','대회 참가신청','/events.php'],['team-line','동호회 찾기','/clubs.php'],['book-open-line','교육 신청','/education.php'],['medal-line','자격검증','/qualification.php'],['hand-heart-line','자원봉사','/volunteers.php'],['id-card-line','모바일 회원증','/mypage.php?tab=card'] ].map(([icon,title,url])=>({icon,title,url}));
...
export const posts = Array.from({length:64},(_,i)=>({...base(`post-${i+1}`,`${postTitles[i%6]}${i>7?` · ${Math.floor(i/8)+1}호`:''}`,boards[i%8]),createdAt:`2026-09-${String(17-i%16).padStart(2,'0')}`,summary:'일과 스포츠가 조화로운 일상, 대한직장인체육회와 함께해요.',body:['이 게시물은 홈페이지 기능을 확인하기 위한 데모 원고입니다. 실제 모집 공고가 아닙니다.','다양한 종목을 직접 경험하고 동료와 교류하는 생활체육 프로그램을 소개합니다. 행사별 일정과 참가 대상은 대회 상세 페이지에서 확인할 수 있습니다.','참여 전 안전수칙과 준비사항을 확인해주세요. 공식 운영 일정 및 안내 자료는 체육회 검토 후 교체됩니다.'],image:mediaAssets[i%4].image,alt:mediaAssets[i%4].alt,pinned:i===0,attachment:i%8===7?'/assets/safety-guide.txt':null}));
export const clubs = Array.from({length:16},(_,i)=>({...base(`club-${i+1}`,['그린메이트 골프클럽','퇴근길 펜싱크루','위켄드 승마클럽','오후 여섯시 FC'][i%4]+(i>=4?` ${regions[Math.floor(i/4)]}`:''),sports[i%4]),sport:sports[i%4],region:regions[Math.floor(i/4)],recruiting:i%5!==4,featured:i<4,memberCount:12+i*2,meeting:['격주 토요일 오전','매주 수요일 저녁','매월 둘째 주 일요일','매주 금요일 저녁'][i%4],organizationId:i%2===0?'org-1':null,associationId:`assoc-${i%4+1}`,image:mediaAssets[i%4].image,alt:mediaAssets[i%4].alt,summary:['스코어보다 함께하는 즐거움, 주말 라운딩 친구를 만나요.','퇴근 후 한 시간, 새로운 취미를 함께 배워요.','도시를 벗어나 말과 교감하는 여유로운 주말.','승패보다 팀워크! 함께 땀 흘릴 동료를 기다려요.'][i%4]}));
export const courses = Array.from({length:8},(_,i)=>({...base(`course-${i+1}`,['스포츠 현장 안전교육','처음 시작하는 골프 레슨','생활체육 지도자 보수교육','펜싱 입문 클래스','자원봉사자 사전 안전교육','승마 기초와 안전수칙','풋살 팀워크와 기본기','지도자를 위한 응급 대응'][i],i%4===0?'안전교육':i===2?'보수교육':'레슨'),image:mediaAssets[i%4].image,alt:mediaAssets[i%4].alt,duration:'약 10분 · 3개 학습단계',format:'온라인 데모',associationId:`assoc-${i%4+1}`,lessonIds:['lesson-1','lesson-2','lesson-3']}));
export const lessons = [{id:'lesson-1',title:'01. 활동 전 준비',body:'활동 전 본인의 건강 상태를 확인하고 충분히 준비운동을 합니다. 장소와 비상 연락망을 먼저 확인하세요.'},{id:'lesson-2',title:'02. 현장 안전수칙',body:'종목별 보호장비를 착용하고 운영자의 안내를 따릅니다. 이상 징후가 있으면 즉시 활동을 멈추고 담당자에게 알립니다.'},{id:'lesson-3',title:'03. 안전 확인',body:'자료를 읽고 아래 확인 질문에 답하면 데모 이수 처리가 가능합니다. 이 페이지는 공인 안전교육을 대체하지 않습니다.'}];
export const qualificationPrograms = [{...base('qual-1','생활체육 운영 실무 자격검증','운영'),summary:'운영 기초와 안전 지식을 확인하는 데모 자격검증',courseId:'course-1'},{...base('qual-2','동호회 리더 역량 검증','지도'),summary:'동호회 운영 및 커뮤니케이션 역량 데모 검증',courseId:'course-3'}];
export const members = [{id:'member-1',name:'김스포츠 (데모)',number:'DEMO-2026-0001',organizationId:'org-1',organizationNameSnapshot:'가상 그린워크',joinedAt:'2026-09-01',status:'활동',interests:['골프','풋살'],isDemo:true},{id:'member-2',name:'이필드 (데모)',number:'DEMO-2026-0002',organizationId:'org-2',organizationNameSnapshot:'가상 플레이랩',joinedAt:'2026-09-02',status:'활동',interests:['펜싱'],isDemo:true}];
...
export const certificates = [{id:'cert-1',memberId:'member-1',programId:'qual-1',title:'생활체육 운영 실무 자격검증',status:'합격',number:'DEMO-CERT-0001',issuedAt:null,isDemo:true}];
export const volunteerPrograms = Array.from({length:6},(_,i)=>({...base(`volunteer-${i+1}`,`${sports[i%4]} 대회 운영 자원봉사 ${i+1}기`,'자원봉사'),eventId:`event-${i+1}`,cohort:`${i+1}기`,roles:['현장 안내','경기 운영 지원','안전 지원'],courseId:'course-5',region:regions[i%4],image:mediaAssets[i%4].image,alt:mediaAssets[i%4].alt}));
...
export const safetyMaterials = [{id:'guide-1',title:'스포츠 현장 안전 가이드 (데모)',url:'/assets/safety-guide.txt'}];
...
const copy = {
 ...
 vision:['일과 삶 사이, 스포츠라는 연결.','누구나 쉽게 참여하고 함께 성장하는 생활체육 환경을 지향합니다.','Vision 1과 Vision 2의 상세 문구는 검토용 제안입니다.'],
 'vision-1':['더 가까운 스포츠','일터와 지역에서 자연스럽게 만나는 생활체육.','접근성 · 참여 기회 · 일상의 균형'],
 'vision-2':['함께 성장하는 스포츠','개인과 동호회, 기업이 함께하는 연결.','교류 · 안전 · 지속 가능한 활동'],
 purpose:['건강한 일상, 함께하는 가치','직장인의 스포츠 참여와 교류를 지원하는 홈페이지 구성입니다.','정관에 따른 법적 설립목적은 공식 원고로 교체합니다.'],
 ...
 ci:['대한직장인체육회','기관명 중심의 임시 워드마크','공식 CI 미제공: 이 페이지의 그린 컬러와 워드마크는 프로젝트 제안입니다.'],
 articles:['공정하고 투명한 운영을 위해','정관 및 운영 규정','법적 효력이 없는 검토용 페이지입니다. 공식 정관과 개정 이력은 검토 후 게시합니다.'],
 directions:['체육회에 오시는 길','주소 및 교통 안내','공식 주소와 전화번호가 제공되지 않아 지도와 연락처를 임의로 표시하지 않습니다.'],
 business:['일상에 스포츠의 가능성을 더합니다','참여 · 교육 · 교류 · 안전','대회와 교육, 동호회 서비스를 통해 나에게 맞는 스포츠를 만나보세요.'],
 reform:['더 열린 체육회를 향한 변화','참여 중심의 서비스 개선','투명한 정보 공개와 쉬운 신청, 접근성 개선을 위한 제안 과제입니다.'],
 operations:['안전하고 즐거운 대회를 만듭니다','대회 운영 안내','공고 확인 → 참가신청 → 접수 확인 → 안전교육 → 대회 참여'],
 talent:['경험이 필요한 곳에, 당신의 역량을','지도자·전문인력 인력풀','관심 분야와 활동 지역을 등록하는 데모 신청입니다.'],
 ...
 qna:['무엇을 도와드릴까요?','Q&A 문의하기','문의 내용은 현재 브라우저 세션에 데모 내역으로만 기록됩니다.'],
 'safety-proposal':['여러분의 제안이 더 안전한 현장을 만듭니다','안전개선제안','장소, 상황과 개선 의견을 남겨주세요. 긴급 신고 접수 기능은 아닙니다.'],
 'members-only':['회원과 함께 나누는 이야기','회원 전용 자료','데모 개인회원으로 전환하여 전용 자료와 참여 이력을 확인하세요.'],
 terms:['서비스 이용약관','검토 필요 원고','이 페이지는 법률 검토와 운영 정책 확정 전의 화면 양식입니다. 실제 서비스 약관 동의로 사용하지 않습니다.'],
 privacy:['개인정보처리방침','검토 필요 원고','현재 프로토타입은 실제 개인정보를 수집하지 않습니다. 합성 데이터만 입력해 주세요.'],
 associations:['다양한 종목, 하나의 연결','산하 종목협회','협회명은 확장 구조를 보여주는 예시이며 공식 명단이 아닙니다.']
};
export const pageContents = Object.fromEntries(Object.entries(copy).map(([id,[headline,subtitle,body]])=>[id,{id,title:navigation.flatMap(n=>n.items.flatMap(x=>[x,...(x.children||[])])).find(x=>x.id===id)?.title||headline,headline,subtitle,body,isDemo:true}]));
export const faqs = [{q:'대회에는 어떻게 참가하나요?',a:'대회·행사에서 접수중인 대회를 선택하고 참가 유형과 정보를 입력한 뒤 신청을 확인하세요. 데모 신청 내역은 마이페이지에 표시됩니다.'},{q:'기업회원과 동호회 등록은 무엇이 다른가요?',a:'기업은 소속 직장이고 동호회는 활동 모임입니다. 별도로 등록하며 동호회 등록 시 기업을 연결할 수 있습니다.'},{q:'신청 후 취소할 수 있나요?',a:'마이페이지 신청 내역에서 데모 신청을 취소할 수 있습니다. 실제 취소·환불 정책은 후속 운영 정책으로 확정합니다.'},{q:'교육 수료는 어떻게 확인하나요?',a:'모든 학습단계 열람, 자료 필독 확인과 안전 질문의 정답 제출을 완료하면 데모 이수가 기록됩니다.'},{q:'이 사이트에서 결제나 문자가 실제로 처리되나요?',a:'아니요. 이 사이트는 프론트 프로토타입입니다. 실제 결제, 인증, 메일 및 SMS 전송은 수행하지 않습니다.'}];
