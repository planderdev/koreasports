...
export const navigationAll = [
 {title:'소개',english:'ABOUT US',items:[page('회장인사말','greeting'),page('연혁','history'),{...page('Vision','vision'),children:[page('개혁과제','reform')]},page('설립목적','purpose'),page('조직구성회','organization'),page('시·도 체육회장','regional'),hidden(page('임원 & 위원회','committee')),page('체육회 CI','ci'),page('오시는길','directions')]},
 {title:'주요사업',english:'OUR BUSINESS',items:[/* 하위 항목은 주요사업 페이지의 분야 순서를 그대로 따릅니다(business-content.js fields). 항목을 누르면 페이지의 해당 분야로 이동합니다. */{...page('주요사업','business'),children:businessContent.business.fields.map(f=>link(f.menu||f.title,`/page.php?id=business#field-${f.id}`))},{...link('대회·행사','/events.php'),children:[page('대회운영','operations'),link('대회참가신청','/events.php?status=접수중'),link('대회소식/공지','/board.php?category=대회공고'),link('현장갤러리','/board.php?category=포토·영상')]},hidden(page('문화예술 육성사업','culture')),hidden({...link('동호회 가입','/clubs.php'),children:[page('복지정책','welfare'),link('클럽/동호회 등록','/join.php?type=club')]})]},
 {title:'교육사업',english:'EDUCATION',items:[{...link('교육사업','/education.php'),children:[link('레슨 & 안전교육','/education.php?category=레슨'),link('보수교육','/education.php?category=보수교육')]},{...link('자격검증','/qualification.php'),children:[link('자격검증신청','/qualification.php?tab=programs'),link('합격조회·발급','/qualification.php?tab=results'),link('재발급신청','/qualification.php?tab=reissue'),page('인재매칭(인력풀)','talent')]},page('안전관리','safety')]},
 {title:'커뮤니티',english:'COMMUNITY',items:[link('공지사항','/board.php?category=공지사항'),link('대회공고','/board.php?category=대회공고'),hidden(link('대회공모','/board.php?category=대회공모')),link('언론·보도','/board.php?category=언론·보도'),link('포토·영상','/board.php?category=포토·영상'),page('자주묻는질문','faq'),link('Q&A','/board.php?category=Q%26A'),link('자료실','/board.php?category=자료실'),hidden(page('정관','articles')),hidden(link('Club Matching','/clubs.php')),hidden(link('우리 동호회 자랑','/board.php?category=동호회 소식')),hidden(link('이달의 우수동호회','/clubs.php?featured=1')),hidden(link('안전개선제안','/board.php?category=안전개선제안')),hidden(link('회원전용','/board.php?category=회원전용'))]},
 {title:'후원',english:'SUPPORT',items:[link('후원사업안내','/support.php?tab=business'),link('후원방법','/support.php?tab=method'),hidden(link('후원안내','/support.php?tab=guide')),link('후원회원예우','/support.php?tab=benefits'),hidden(link('후원문의','/support.php?tab=contact')),link('자원봉사 참여하기','/volunteers.php'),hidden({title:'후원하기(KSTMA 참고)',url:'http://www.kstma.com/',external:true}),hidden(link('후원 참여 문의','/support.php?tab=donate')),hidden(link('후원금집행내역','/support.php?tab=reports')),hidden(link('기업파트너십','/support.php?tab=partnership')),hidden(link('모집안내/권리','/support.php?tab=rights')),hidden(link('후원사현황','/support.php?tab=sponsors'))]}
];
// 홈 하단 후원 배너에 표시할 후원 메뉴 항목(제목 기준, 이 순서대로). 메뉴에서 숨기면 배너에서도 빠집니다.
export const homeSupportLinks = ['후원사업안내','후원회원예우','자원봉사 참여하기'];
...
export const registrationPlans = [
 {id:'basic',title:'기본형',tagline:'안전하게 시작하는 선수 생활',price:'50,000원',unit:'연',benefits:['필수 스포츠상해보험 자동 가입','체육회 주관 대회 참가비 10~20% 할인','모바일 선수증 발급'],highlight:'연 3회 대회에 참가하면 할인만으로 등록비에 상당하는 금액을 돌려받는 효과가 있습니다.'},
 {id:'premium',title:'프리미엄형',tagline:'더 넓은 보장, 더 많은 혜택',price:'100,000원',unit:'연',benefits:['보장 범위가 확대된 프리미엄 상해보험','주관 대회 참가비 우대 할인','제휴 스포츠 용품 할인 쿠폰북 제공','모바일 선수증 발급'],highlight:'대회에 자주 참가하거나 더 든든한 보장을 원하는 선수에게 알맞습니다.'}
];
// 등록 유형 위에 보여 주는 안내: 등록비가 어디에 쓰이고 등록 선수가 무엇을 받는지.
export const registrationPlanIntro = '선수등록비는 단체 상해보험 가입과 대회·교육 운영에 쓰입니다. 등록 선수는 유형에 관계없이 아래 혜택을 받고, 유형에 따라 보장 범위와 우대 혜택이 더해집니다.';
export const registrationPlanCommon = ['스포츠 단체 상해보험 자동 가입','모바일 선수증 발급','주관 대회 참가비 할인','체육회 복지몰 이용 (시중가 대비 10% 할인)'];
...
// 선수등록 폼의 소재지 시·도. 시·군·구·읍·면·동 목록은 /assets/data/korea-districts.json(행정동 기준, 2026-07-01)에 있고,
// 이 목록은 그 파일의 시·도 키와 같아야 합니다(광주·전남은 통합특별시로 표기).
export const provinces = ['서울특별시','부산광역시','대구광역시','인천광역시','전남광주통합특별시','대전광역시','울산광역시','세종특별자치시','경기도','강원특별자치도','충청북도','충청남도','경상북도','경상남도','제주특별자치도','전북특별자치도'];
...
// 체육회가 직접 올리는 게시글(원문 이관분과 구분). 자료실의 정관 게시글은 정관 파일을 받으면 attachment 에 연결합니다.
export const localPosts = [
 {id:'post-articles',slug:'post-articles',title:'대한직장인체육회 정관',category:'자료실',sport:'',summary:'대한직장인체육회 정관 전문',body:['대한직장인체육회 정관입니다.','정관 전문과 개정 이력은 첨부 파일로 게시합니다.'],createdAt:'2026-09-22',updatedAt:'2026-09-22',status:'published',pinned:true,author:'대한직장인체육회',views:0,image:'/assets/images/fallback.svg',alt:'',attachment:null,showLeadImage:false,isDemo:true}
];
export const posts = [...sourcePosts, ...localPosts];
