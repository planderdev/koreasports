...
export function variants(d){const id=d.id;
 ...
 if(['chip','filter-button','segmented-control','category'].includes(id))return `<div class="ds-flex"><button class="ds-button ds-chip" aria-pressed="false" data-variant-toggle>기본</button><button class="ds-button ds-chip" aria-pressed="true" data-variant-toggle>선택</button><button class="ds-button ds-chip" disabled>비활성</button></div>`;
 ...
 if(['button','icon-button','text-button','action-area'].includes(id))return `<div class="ds-flex">${['sm','md','lg'].map(n=>`<button class="ds-button" data-size="${n}" data-variant-toast>${n.toUpperCase()}</button>`).join('')}<button class="ds-button ds-outline" data-variant-toast>Outlined</button><button class="ds-button ds-text" data-variant-toast>Text</button></div>`;
 return '';
}
export function implementationCode(d){const imports={
 ...
 };return `<section class="ds-doc-section" id="implementation"><h2>Implementation code</h2><p>스타일은 tokens.css와 design-system-docs.css를 함께 로드합니다. 아래 예제는 프로젝트의 공통 dialog·toast 영역이 있는 페이지를 기준으로 합니다. 예제 ID가 겹치지 않도록 한 화면에는 각 예제를 한 번씩 마운트합니다.</p><pre><code>${esc(imports[d.id]||`import {${d.section==='utilities'?'renderUtility':'renderComponent'}, bindExamples} from '/assets/js/design-system/components.js';\n\nconst root = document.querySelector('#example');\nroot.innerHTML = ${d.section==='utilities'?'renderUtility':'renderComponent'}('${d.id}');\nbindExamples(root);`)}</code></pre></section>`;
}
