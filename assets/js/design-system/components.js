...
const btn=(text,extra='',cls='')=>`<button type="button" class="ds-button ${cls}" ${extra}>${text}</button>`;
const input=(type='text',extra='')=>`<input class="ds-input" type="${type}" ${extra}>`;
const avatars=()=>`<div class="ds-avatars" aria-label="참가자: 김민수, 이서연, 박지훈 외 3명">${['김','이','박','+3'].map((n,i)=>`<span class="ds-avatar" style="--avatar-color:var(--semantic-accent-${['green','blue','violet','orange'][i]}-subtle)">${n}</span>`).join('')}</div>`;
...
const field=()=>`<form data-example-form novalidate><label for="example-name">이름 <span aria-hidden="true">*</span></label>${input('text','id="example-name" name="name" required autocomplete="off" placeholder="이름을 입력하세요" aria-describedby="example-help example-error"')}<p id="example-help" class="ds-help">입력값은 저장하거나 전송하지 않습니다.</p><p id="example-error" class="ds-error" aria-live="polite"></p><button class="ds-button" type="submit">입력 확인</button></form>`;
...
export function renderComponent(id){switch(id){
 ...
 case 'list-card':return `<a class="ds-list-card" href="/event.php?id=event-1"><img src="/assets/images/golf.webp" alt="골프 코스"><div><span class="ds-badge">골프</span><h3>2026 직장인 그린컵</h3><p>10월 10일 · 서울 · 데모 행사</p></div>${icon('arrow-right-s-line')}</a>`;
 ...
 case 'section-header':return `<div class="ds-section-header"><div><p class="ds-help">SPORTS & PEOPLE</p><h3>함께 도전하는 대회</h3><p>다음 일정을 만나보세요.</p></div><a class="ds-button ds-text" href="/events.php">전체보기 ${icon('arrow-right-line')}</a></div>`;
 case 'table':return `<div class="ds-table-scroll" tabindex="0" role="region" aria-label="대회 현황 표"><table class="ds-table"><caption>시연 대회 접수 현황</caption><thead><tr><th scope="col">대회명</th><th scope="col">종목</th><th scope="col">상태</th></tr></thead><tbody>${[['그린컵','골프','접수중'],['퇴근 후 챌린지','펜싱','접수중'],['가을 페스티벌','승마','접수예정']].map(x=>`<tr>${x.map(t=>`<td>${t}</td>`).join('')}</tr>`).join('')}</tbody></table></div>`;
 ...
 case 'search-field':return `<form data-example-search><label for="example-search">종목 검색</label><div class="ds-search-input">${input('search','id="example-search" placeholder="골프, 펜싱, 승마, 풋살"')}<button class="ds-button ds-icon" aria-label="검색 실행">${icon('search-line')}</button></div><p class="ds-result" role="status">검색어를 입력해주세요.</p></form>`;
...
export function renderUtility(id){
 ...
 case 'ellipsis-typography-style':return '<div class="ds-box" style="max-width:var(--size-280px)"><p class="ds-ellipsis">한 줄로 요약되는 긴 제목입니다. 원문은 DOM에 유지됩니다.</p><hr><p class="ds-clamp">대한직장인체육회는 일상 속 스포츠 활동을 통해 건강한 내일을 만들어갑니다. 함께 참여하고 배우는 즐거움을 경험해보세요. 두 줄 이후는 줄임표로 표시합니다.</p></div>';
 ...
 case 'navigation':return '<nav class="ds-flex" aria-label="사이트 탐색 예시"><a class="ds-button ds-outline" href="/events.php">대회 목록</a><a class="ds-button ds-outline" href="/education.php">교육 목록</a></nav>';
...
export function bindExamples(root){
 ...
 on('[data-demo-toast]','click',()=>toast('동작을 확인했습니다. 데모 상태만 변경됩니다.'));
 ...
 on('[data-open]','click',(_,el)=>{const kind=el.dataset.open;const d=modal(kind==='sheet'?'참가 방식 선택':kind==='video'?'축구 훈련 영상':'내용 확인',`<div class="ds-demo">${kind==='video'?'<video controls playsinline class="ds-video" src="/assets/videos/field-training.mp4"></video><p>스톡 영상 · 실제 행사 기록이 아닙니다.</p>':`<p>시연용 대화상자입니다. Tab으로 내부를 이동하고 Escape로 닫아보세요.</p><div class="ds-action-area">${btn('취소','data-close','ds-outline')}${btn('확인','data-close')}</div>`}</div>`);d.classList.toggle('ds-bottom-sheet',kind==='sheet');d.dataset.dsTheme=document.getElementById('ds-root')?.dataset.dsTheme||'light';});
 ...
 on('[data-menu-action]','click',(_,el)=>{root.querySelector('.ds-result').textContent=el.textContent+' 완료 (시연)';document.getElementById('example-popover').hidePopover();});
