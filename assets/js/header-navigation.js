...
// External menu links (external:true) open in a new tab.
const ext=item=>item.external?' target="_blank" rel="noopener noreferrer"':'';
export function setupHeaderNavigation(nav){
 ...
 gnb.innerHTML=nav.map((n,i)=>`<div class="gnb-item"><button data-nav="${i}" aria-expanded="false" aria-controls="nav-panel-${i}">${esc(n.title)}</button><div class="gnb-panel" id="nav-panel-${i}" hidden><nav aria-label="${esc(n.title)} 하위 메뉴">${n.items.map((item,j)=>item.children?.length?`<div class="nav-branch"><div class="nav-branch-row"><a href="${item.url}"${ext(item)}>${esc(item.title)}</a><button class="nav-expand" data-subnav aria-expanded="false" aria-controls="nav-third-${i}-${j}" aria-label="${esc(item.title)} 3차 메뉴">${icon('arrow-right-s-line')}</button></div><div class="nav-third" id="nav-third-${i}-${j}" hidden>${item.children.map(child=>`<a href="${child.url}"${ext(child)}>${esc(child.title)}</a>`).join('')}</div></div>`:`<a href="${item.url}"${ext(item)}>${esc(item.title)}</a>`).join('')}</nav></div></div>`).join('');
