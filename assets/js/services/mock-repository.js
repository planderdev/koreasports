...
/* 선수등록 때 만든 계정으로만 로그인됩니다. 결과: 'ok' | 'no-account' | 'mismatch' */
export function login(email,passwordHash){const a=state().account;if(!a)return 'no-account';if(a.email!==String(email||'').trim().toLowerCase()||a.passwordHash!==String(passwordHash||'').toLowerCase())return 'mismatch';setRole('member');return 'ok';}
export function getAccount(){const a=state().account;return a?{email:a.email,name:a.name,createdAt:a.createdAt}:null;}
...
export function register(type,payload){const s=state();if(type==='member'){s.role='member';const sports=cleanSports(payload?.sports);/* 등록 때 입력한 참여 종목·연락처·소재지·등록 유형을 로그인 회원(members[0])의 프로필에 반영합니다. 허용한 값만 받고, 주민등록번호·여권번호 등 그 밖의 입력은 저장하지 않습니다. */const profile={};/* 이름·소속 직장·가입일도 로그인 회원 프로필로 덮어써서 마이페이지·회원증에 데모 회원(김스포츠) 값이 남지 않게 합니다. 소속 직장을 비우면 '등록된 정보가 없습니다'로 표시되고, 동호회 소속은 등록 직후 없는 상태(clubIds: [])로 시작합니다. */const name=typeof payload?.name==='string'?payload.name.trim().slice(0,40):'';if(name)profile.name=name;const org=typeof payload?.organizationName==='string'?payload.organizationName.trim().slice(0,80):'';profile.organizationNameSnapshot=org;profile.organizationId=data.organizations.find(o=>o.title===org)?.id||null;profile.clubIds=[];/* 회원번호는 데모 회원 다음 번호를 새로 부여하고(KOWSC-연도-일련번호), 데모 회원 1번에 붙어 있던 자격·신청·학습 기록은 새 회원 것이 아니므로 비웁니다. */profile.number=`KOWSC-${new Date().getFullYear()}-${String(data.members.length+1).padStart(4,'0')}`;const me=data.members[0].id;s.certificates=s.certificates.filter(c=>c.memberId!==me);s.applications=s.applications.filter(a=>a.memberId!==me);s.progress=s.progress.filter(p=>p.memberId!==me);profile.joinedAt=(d=>`${d.getFullYear()}-${String(d.getMonth()+1).padStart(2,'0')}-${String(d.getDate()).padStart(2,'0')}`)(new Date());/* 한국 시간 기준 날짜(UTC로 자르면 자정 전후 하루가 밀립니다) */if(sports.length)profile.interests=sports;if(typeof payload?.phone==='string'&&/^01[016789]-\d{3,4}-\d{4}$/.test(payload.phone))profile.phone=payload.phone;if(data.provinces.includes(payload?.province))profile.province=payload.province;const place=v=>typeof v==='string'&&/^[가-힣0-9·\s]{1,40}$/.test(v.trim())?v.trim():null;if(profile.province){profile.district=place(payload?.district);profile.town=place(payload?.town);}/* 로그인 계정: 이메일(ID)과 비밀번호의 SHA-256 해시만 저장하고, 비밀번호 원문은 받지 않습니다. */if(typeof payload?.email==='string'&&/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(payload.email)&&/^[a-f0-9]{64}$/i.test(payload?.passwordHash||''))s.account={email:payload.email.trim().toLowerCase(),passwordHash:payload.passwordHash.toLowerCase(),name:typeof payload.name==='string'?payload.name.trim().slice(0,40):'',createdAt:new Date().toISOString()};if(data.registrationPlans.some(p=>p.id===payload?.plan))profile.plan=payload.plan;if(Object.keys(profile).length){const key=`members:${data.members[0].id}`;s.edits[key]={...(s.edits[key]||{}),...profile};}s.notifications.push({id:crypto.randomUUID(),templateId:'welcome',body:data.notificationTemplates[0].body,status:'미리보기',createdAt:new Date().toISOString()});}else{s.registrations.push({id:`registration-${crypto.randomUUID()}`,type,title:type==='club'?'신규 동호회':'신규 기업',status:'승인대기',region:data.regions.includes(payload.region)?payload.region:'서울',sport:data.sports.includes(payload.sport)?payload.sport:(Array.isArray(payload.sports)&&typeof payload.sports[0]==='string'&&payload.sports[0].trim()?payload.sports[0].trim().slice(0,40):'골프'),sports:cleanSports(payload.sports),organizationId:payload.organizationId==='org-1'?'org-1':null,createdAt:new Date().toISOString(),isDemo:true});s.role='manager';}write(s);}
