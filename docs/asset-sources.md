...
`assets/data/korea-districts.json` — 시·도 → 시·군·구 → 읍·면·동 이름 목록(16개 시·도, 3,558개 행정동). 출처: vuski/admdongkor `ver20260701/HangJeongDong_ver20260701.geojson`(https://github.com/vuski/admdongkor, 데이터 라이선스 CC BY 4.0; 원자료 행정안전부 행정동 경계)에서 `adm_nm`만 추출해 이름으로 정리했고, 경계 좌표는 포함하지 않습니다. 시·군·구는 "수원시 장안구"처럼 띄어 씁니다. 이 데이터 기준으로 광주광역시·전라남도는 "전남광주통합특별시"로 표기됩니다. 갱신 시 같은 저장소의 최신 버전으로 다시 추출합니다.
